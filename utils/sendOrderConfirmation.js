...
const sendOrderConfirmation = async (orderId) => {
    ...
    `;
        ...
        if (userEmail) {
            ...
            await sendEmail(userEmail, `Order Confirmation - #${order._id}`, customerEmailHTML);
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #333; border-radius: 8px;">
        <h2 style="color: #2196F3; text-align: center;">💰 New Order Received (${paymentMethodName})</h2>
        
        <div style="background: #e3f2fd; padding: 15px; margin: 15px 0; border-radius: 5px;">
            <p style="margin: 5px 0;"><strong>Order ID:</strong> ${order._id}</p>
            <p style="margin: 5px 0;"><strong>Status:</strong> ${order.paymentStatus.toUpperCase()}</p>
            <p style="margin: 5px 0;"><strong>Total Amount:</strong> ${displayCurrency} ${displayTotal}</p>
        </div>

        <h3>Customer Details</h3>
        <p><strong>Name:</strong> ${order.billingAddress?.firstName || order.shippingAddress?.firstName || "Customer"} ${order.billingAddress?.lastName || order.shippingAddress?.lastName || ""}</p>
        <p><strong>Email:</strong> ${userEmail}</p>
        <p><strong>Phone:</strong> ${order.billingAddress?.phone || order.shippingAddress?.phone || "N/A"}</p>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            ...
            <tfoot>
                <tr>
                    ...
                    <td style="padding: 10px; text-align: right;"><strong>${displayCurrency} ${displayTotal}</strong></td>
                </tr>
            </tfoot>
        </table>

        <div style="background: #fff3cd; padding: 10px; border-radius: 4px; border: 1px solid #ffeeba; color: #856404;">
            <p style="margin: 0;"><strong>Action Required:</strong> Please proceed with fulfillment.</p>
        </div>
      </div>
    `;
        ...
        if (process.env.ADMIN_EMAIL) {
            ...
            await sendEmail(process.env.ADMIN_EMAIL, `💰 New Order #${order._id} - ${displayCurrency} ${displayTotal}`, adminEmailHTML);
            console.log(`✅ Admin notification sent to ${process.env.ADMIN_EMAIL}`);
        }
        if (process.env.SALES_EMAIL) {
            console.log(`📨 Attempting to send sales notification to: ${process.env.SALES_EMAIL}`);
            await sendEmail(process.env.SALES_EMAIL, `💰 New Order #${order._id}`, adminEmailHTML);
