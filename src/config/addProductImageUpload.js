        ...
        uploader(req, res, async (err) => {
            ...
            try {
                ...
                if (req.files?.main) {
                    ...
                    uploadedImages.push({ url: mainResult.secure_url, type: "main", alt: req.files.main[0].originalname });
                ...
                if (req.files?.covers) {
                    for (const file of req.files.covers) {
                        ...
                        uploadedImages.push({ url: result.secure_url, type: "cover", alt: file.originalname });
                        fs.unlink(file.path, () => {});
                    }
                }
    ...
    module.exports = addProductImageUpload; // ✅ make sure it's a function
